...
static int fault_get_reg(struct bt_mesh_model* model, uint16_t cid, uint8_t* test_id, uint8_t* faults, uint8_t* fault_count) {
  if (cid != CID_VENDOR) {
...
static void
health_current_status(struct bt_mesh_health_cli* cli, uint16_t addr, uint8_t test_id, uint16_t cid, uint8_t* faults, size_t fault_count) {
...
static struct bt_mesh_model vnd_models[] = {
  BT_MESH_MODEL_VND(CID_VENDOR, VND_MODEL_ID_1, BT_MESH_MODEL_NO_OPS, NULL, NULL),
};
...
struct shell_cmd_help cmd_static_oob_help = {NULL, "[val: 1-16 hex values]", NULL};
...
static int check_pub_addr_unassigned(void) {
  ...
  return memcmp(MYNEWT_VAL(BLE_PUBLIC_DEV_ADDR), zero_addr, BLE_DEV_ADDR_LEN) == 0;
  #endif
}
...
struct shell_cmd_help cmd_iv_update_test_help = {NULL, "<value: off, on>", NULL};
  #endif
...
static void print_unprovisioned_beacon(uint8_t uuid[16], bt_mesh_prov_oob_info_t oob_info, uint32_t* uri_hash) {
  char uuid_hex_str[32 + 1];

  bin2hex(uuid, 16, uuid_hex_str, sizeof(uuid_hex_str));

  printk("UUID %s, OOB Info 0x%04x, URI Hash 0x%lx", uuid_hex_str, oob_info, (uri_hash == NULL ? 0 : *uri_hash));
}
...
static int cmd_net_transmit(int argc, char* argv[]) {
  ...
  printk("Transmit 0x%02x (count %u interval %ums)", transmit, BT_MESH_TRANSMIT_COUNT(transmit), BT_MESH_TRANSMIT_INT(transmit));

  return 0;
}

struct shell_cmd_help cmd_net_transmit_help = {NULL, "[<count: 0-7> <interval: 10-320>]", NULL};
...
struct shell_cmd_help cmd_relay_help = {NULL, "[val: off, on] [count: 0-7] [interval: 0-32]", NULL};
...
struct shell_cmd_help cmd_net_key_add_help = {NULL, "<NetKeyIndex> [val]", NULL};
...
struct shell_cmd_help cmd_app_key_add_help = {NULL, "<NetKeyIndex> <AppKeyIndex> [val]", NULL};
...
struct shell_cmd_help cmd_app_key_del_help = {NULL, "<NetKeyIndex> <AppKeyIndex>", NULL};
...
struct shell_cmd_help cmd_mod_app_bind_help = {NULL, "<addr> <AppIndex> <Model ID> [Company ID]", NULL};
...
static int cmd_mod_app_unbind(int argc, char* argv[]) {
  ...
  if (argc > 4) {
    cid = strtoul(argv[4], NULL, 0);
    err = bt_mesh_cfg_mod_app_unbind_vnd(net.net_idx, net.dst, elem_addr, mod_app_idx, mod_id, cid, &status);
  } else {
    err = bt_mesh_cfg_mod_app_unbind(net.net_idx, net.dst, elem_addr, mod_app_idx, mod_id, &status);
  }
...
struct shell_cmd_help cmd_mod_app_unbind_help = {NULL, "<addr> <AppIndex> <Model ID> [Company ID]", NULL};
...
static int cmd_mod_app_get(int argc, char* argv[]) {
  ...
  if (argc > 3) {
    ...
    err = bt_mesh_cfg_mod_app_get_vnd(net.net_idx, net.dst, elem_addr, mod_id, cid, &status, apps, &cnt);
  } else {
    err = bt_mesh_cfg_mod_app_get(net.net_idx, net.dst, elem_addr, mod_id, &status, apps, &cnt);
  }
  ...
  if (status) {
    ...
    printk("Apps bound to Element 0x%04x, Model 0x%04x %s:", elem_addr, mod_id, argc > 3 ? argv[3] : "(SIG)");
...
struct shell_cmd_help cmd_mod_app_get_help = {NULL, "<elem addr> <Model ID> [Company ID]", NULL};
...
struct shell_cmd_help cmd_mod_sub_add_help = {NULL, "<elem addr> <sub addr> <Model ID> [Company ID]", NULL};
...
struct shell_cmd_help cmd_mod_sub_del_help = {NULL, "<elem addr> <sub addr> <Model ID> [Company ID]", NULL};
...
struct shell_cmd_help cmd_mod_sub_add_va_help = {NULL, "<elem addr> <Label UUID> <Model ID> [Company ID]", NULL};
...
struct shell_cmd_help cmd_mod_sub_del_va_help = {NULL, "<elem addr> <Label UUID> <Model ID> [Company ID]", NULL};
...
static int cmd_mod_sub_get(int argc, char* argv[]) {
  ...
  if (argc > 3) {
    cid = strtoul(argv[3], NULL, 0);
    err = bt_mesh_cfg_mod_sub_get_vnd(net.net_idx, net.dst, elem_addr, mod_id, cid, &status, subs, &cnt);
  } else {
    err = bt_mesh_cfg_mod_sub_get(net.net_idx, net.dst, elem_addr, mod_id, &status, subs, &cnt);
  }
...
struct shell_cmd_help cmd_mod_sub_get_help = {NULL, "<elem addr> <Model ID> [Company ID]", NULL};
...
struct shell_cmd_help cmd_mod_pub_help = {NULL,
...
struct shell_cmd_help cmd_hb_pub_help = {NULL, "<dst> <count> <period> <ttl> <features> <NetKeyIndex>", NULL};
...
struct shell_cmd_help cmd_provision_adv_help = {NULL, "<UUID> <NetKeyIndex> <addr> <AttentionDuration>", NULL};
...
struct shell_cmd_help cmd_provision_help = {NULL, "<NetKeyIndex> <addr> [IVIndex]", NULL};
...
static int cmd_fault_get(int argc, char* argv[]) {
  ...
  err = bt_mesh_health_fault_get(net.dst, net.app_idx, cid, &test_id, faults, &fault_count);
  if (err) {
...
struct shell_cmd_help cmd_fault_test_help = {NULL, "<Company ID> <Test ID>", NULL};
...
static int cmd_fault_test_unack(int argc, char* argv[]) {
  ...
  err = bt_mesh_health_fault_test(net.dst, net.app_idx, cid, test_id, NULL, NULL);
  if (err) {
...
struct shell_cmd_help cmd_fault_test_unack_help = {NULL, "<Company ID> <Test ID>", NULL};
...
static int cmd_period_set(int argc, char* argv[]) {
  ...
  err = bt_mesh_health_period_set(net.dst, net.app_idx, divisor, &updated_divisor);
  if (err) {
...
struct shell_cmd_help cmd_cdb_subnet_add_help = {NULL, "<NeyKeyIdx> [<NetKey>]", NULL};
struct shell_cmd_help cmd_cdb_subnet_del_help = {NULL, "<NetKeyIdx>", NULL};
struct shell_cmd_help cmd_cdb_app_key_add_help = {NULL, "<NetKeyIdx> <AppKeyIdx> [<AppKey>]", NULL};
...
static void cdb_print_nodes(void) {
  ...
  for (i = 0; i < ARRAY_SIZE(bt_mesh_cdb.nodes); ++i) {
    ...
    printk("0x%04x   %-8d  %-5s  %s  %s", node->addr, node->num_elem, configured ? "C" : "-", uuid_hex_str, key_hex_str);
  }
...
static void cdb_print_app_keys(void) {
  ...
  for (i = 0; i < ARRAY_SIZE(bt_mesh_cdb.app_keys); ++i) {
    ...
    printk("0x%03x   0x%03x   %s", app_key->net_idx, app_key->app_idx, key_hex_str);
  }
...
static void bt_mesh_shell_task_init(void) {
  ...
  os_task_init(&mesh_shell_task,
               "mesh_sh",
               mesh_shell_thread,
               NULL,
               BLE_MESH_SHELL_TASK_PRIO,
               OS_WAIT_FOREVER,
